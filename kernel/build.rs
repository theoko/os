...
fn build_atlas() -> String {
    ...
    // Codepoint range covered by every face (runtime glyph_for / tests).
    let _ = writeln!(out, "pub(crate) const FIRST: u8 = {FIRST:#x};");
    let _ = writeln!(out, "pub(crate) const LAST: u8 = {LAST:#x};\n");

    for style in STYLES {
        rasterize(&data, style, &mut out);
    }
    out
}

/// Rasterize one style into a packed alpha atlas and emit its Rust consts.
fn rasterize(data: &[u8], style: &Style, out: &mut String) {
...
fn build_corpus(manifest_dir: &Path) -> String {
    ...
    let docs = val["docs"].as_array().map(|a| a.as_slice()).unwrap_or(&[]);
    ...
    let lens: Vec<usize> = doc_tokens.iter().map(|t| t.len().max(1)).collect();
    // term -> doc -> tf (consume tokens so keys move, not clone).
    let mut index: BTreeMap<String, BTreeMap<usize, u32>> = BTreeMap::new();
    for (i, toks) in doc_tokens.into_iter().enumerate() {
        for t in toks {
            *index.entry(t).or_default().entry(i).or_insert(0) += 1;
        }
    }
    ...
    for (term, docs_for_term) in &index {
        ...
        for (doc, tf) in docs_for_term {
            ...
            let tfn = ((*tf as f64) / (lens[*doc] as f64) * 65536.0).round() as i64;
