...
pub fn detect_pl011(hhdm: u64) -> Option<usize> {
    // Pass 1: whatever the bootloader already left reachable — through the
    // higher-half direct map first, then raw.
    if let Some(found) = probe_pl011_candidates(hhdm) {
        return Some(found);
    }

    // Pass 2: nothing answered, which is the normal case on QEMU `virt` —
    // Limine's HHDM maps RAM, not device MMIO, and the low physical window is
    // not mapped either, so neither candidate was even readable. Installing
    // the identity-mapped low device window is what turns a physical device
    // address back into a pointer; it is the same window the PCI/ECAM path
    // needs later, and it is a no-op on hosted builds. Only then is a raw
    // probe both safe and meaningful.
    if crate::arm64_mmio::install_low_device_window() {
        return probe_pl011_candidates(hhdm);
    }
    None
}

#[cfg(target_arch = "aarch64")]
fn probe_pl011_candidates(hhdm: u64) -> Option<usize> {
    for base in Serial::PL011_CANDIDATES {
        for candidate in [(hhdm as usize).wrapping_add(base), base] {
