...
# Built-in corpus: search can surface os://; open is ungated (no wire bits).
builtin = call("CALL search.query q=Agent-centric k=5")
if "os://" not in builtin:
    print("error: search.query must surface an os:// hit for Agent-centric", file=sys.stderr)
    print(builtin, file=sys.stderr)
    sys.exit(1)
# Pin the identity card URL so ranking cannot pick a sibling playbook body.
opened_os = call("CALL doc.read url=os://AGENTS.md lines=8")
...
if "Capability-based" not in opened_os and "Agent-centric" not in opened_os:
    print("error: doc.read os://AGENTS.md must return curated body text", file=sys.stderr)
    print(opened_os, file=sys.stderr)
    sys.exit(1)
