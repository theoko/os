...
fn main() {
    ...
    // Prewarm the corpus off the startup path. Lazily, whichever query arrives
    // first pays ~10s while every other one is instant. But doing it *inline*
    // was worse: the port stayed closed for those 10s, so `make utm-bridged`
    // started QEMU against a socket nobody was listening on yet and QEMU
    // aborted with "Connection refused" — a boot failure caused entirely by
    // indexing order. Bind first; warm in the background.
...
fn dispatch(line: &str, backends: &Backends) -> Vec<String> {
    ...
    match cmd {
        ...
        "LIST" => {
            vec!["OK tools=email.search,email.send,calendar.list,skills.list,skills.get,skills.save,search.query,agent.plan,workspace.index,tsearch.sync,market.health,market.fear_greed,audio.transcribe,workspace.forget,audio.forget,portal.forget,portal.status,doc.read".into()]
        }
        "CALL" => {
...
fn read_doc(url: &str, max: usize, args: &[(String, String)]) -> Result<Vec<String>, String> {
    ...
    let body = if let Some(rel) = url.strip_prefix("file://") {
        ...
        // Mail is readable only with the grant that surfaced it.
        if !matches!(arg_val(args, "email"), Some("1")) {
...
fn email_search_mock(query: &str, max: usize) -> Vec<String> {
    ...
    for (from, subj) in samples.iter().take(n) {
        // Carry the graph id so the guest can open the message, not just
        // list it. Same hash the graph uses, so the two agree.
        out.push(format!(
            "ROW from={from}|subj={subj}|url=email://{}",
            graph::id_for(from, subj)
        ));
    }
    out.push("END".into());
    out
